...
rule all:
    input:
        rse_list = expand("results/data/rnaseq/{TOOL}_{ref_build}.{ref_version}/rse_list.RData",
            TOOL= [f"kallisto_v{KALLISTO_version}", f"salmon_v{SALMON_version}"],
            ref_build=ref_build,
            ref_version=ref_version,
        ),
        rsem_rse_list = expand("results/data/rnaseq/rsem-star_v{RSEM_VERSION}-v{STAR_VERSION}/{ref_build}.{ref_version}/rse_list.RData",
            RSEM_VERSION=RSEM_VERSION,
            STAR_VERSION=STAR_VERSION,
            ref_build=ref_build[0],
...
rule create_rse_list_RSEM:
    ...
    output:
        # rse_list = "results/data/rnaseq/rsem-star_v{RSEM_VERSION}-v{STAR_VERSION}/{ref_build}.{ref_version}/rse_list.RData",
    ...
    input:
        quant_files = lambda wc:
            expand(
                ...
                TOOL= wc.TOOL, TOOL_version= wc.TOOL_version,
                ref_build=wc.ref_build, ref_version=wc.ref_version,
                sample= SAMPLES,  
                file = tool_dict[wc.TOOL],
            ),
        annotation_file = lambda wc: expand(
            "{REFERENCE_DIR}/human/{ref_build}_v{ref_version}/annotation.gtf",
            REFERENCE_DIR=REFERENCE_DIR,
